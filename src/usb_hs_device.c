...
#define USB_HS_DEVICE_SETUP_PACKET_SIZE_BYTES                   8

#define USB_HS_DEVICE_DISCONNECT_DELAY_US                       10000

#define USB_HS_DEVICE_NUMBER_OF_LOGICAL_ENDPOINTS               6
#define USB_HS_DEVICE_NUMBER_OF_PHYSICAL_ENDPOINTS              (USB_HS_DEVICE_NUMBER_OF_LOGICAL_ENDPOINTS << 1)
...
#define USB_HS_DEVICE_RAM_EP_CS_SIZE                            (USB_HS_DEVICE_NUMBER_OF_ENDPOINT_BUFFERS << 1)
...
#define USB_HS_DEVICE_RAM_EP_CS_INDEX(ep_num, ep_dir, ep_buf)   ((USB_HS_DEVICE_RAM_EP_CS_SIZE * ep_num) + (ep_dir << 1) + ep_buf)
...
#define USB_HS_DEVICE_EP_PHY(ep_num, ep_dir)                    ((ep_num << 1) + (ep_dir))
#define USB_HS_DEVICE_EP_NUM(ep_phy)                            (ep_phy >> 1)
#define USB_HS_DEVICE_EP_DIR(ep_phy)                            (ep_phy % 2)
...
/*******************************************************************/
typedef enum {
    USB_HS_DEVICE_ENDPOINT_BUFFER_STATE_FREE = 0,
    USB_HS_DEVICE_ENDPOINT_BUFFER_STATE_HW,
    USB_HS_DEVICE_ENDPOINT_BUFFER_STATE_LAST
} USB_HS_DEVICE_endpoint_buffer_state_t;
...
/*******************************************************************/
typedef struct {
    USB_HS_DEVICE_setup_cb_t setup_callback;
    volatile uint32_t intstat;
    volatile USB_HS_DEVICE_flags_t flags;
    volatile USB_HS_DEVICE_physical_endpoint_t physical_endpoints[USB_HS_DEVICE_NUMBER_OF_PHYSICAL_ENDPOINTS];
...
static USB_HS_DEVICE_endpoint_buffer_t _USB_HS_DEVICE_get_endpoint_buffer(uint8_t ep_phy, USB_HS_DEVICE_endpoint_buffer_state_t buffer_state);
    ...
    for (ep_phy = 0; ep_phy < USB_HS_DEVICE_NUMBER_OF_PHYSICAL_ENDPOINTS; ep_phy++) {
        ...
        if (((usb_hs_device_ctx.intstat) & (usb_hs_device_ctx.physical_endpoints[ep_phy].irq_mask)) != 0) {
            ...
            if (usb_hs_device_ctx.physical_endpoints[ep_phy].registered != 0) {
                ...
                switch (ep_phy) {
                case (USB_HS_DEVICE_EP_PHY(0, USB_HS_DEVICE_ENDPOINT_DIRECTION_OUT)):
                    _USB_HS_DEVICE_control_endpoint_out_irq_callback(ep_phy);
                    break;
                case (USB_HS_DEVICE_EP_PHY(0, USB_HS_DEVICE_ENDPOINT_DIRECTION_IN)):
...
static void _USB_HS_DEVICE_control_endpoint_out_irq_callback(uint8_t ep_phy) {
    // Local variables.
    uint8_t ep0_out_cs_index = USB_HS_DEVICE_RAM_EP_CS_INDEX(0, USB_HS_DEVICE_ENDPOINT_DIRECTION_OUT, USB_HS_DEVICE_ENDPOINT_BUFFER_0);
    uint8_t ep0_in_cs_index = USB_HS_DEVICE_RAM_EP_CS_INDEX(0, USB_HS_DEVICE_ENDPOINT_DIRECTION_IN, USB_HS_DEVICE_ENDPOINT_BUFFER_0);
    USB_HS_DEVICE_request_operation_t request_operation = USB_HS_DEVICE_REQUEST_OPERATION_NOT_SUPPORTED;
    // Check setup flag.
    if (((USB_HS_DEVICE->DEVCMDSTAT) & (0b1 << 8)) != 0) {
        ...
        case USB_HS_DEVICE_REQUEST_OPERATION_READ:
            // Start IN data phase.
            ...
            break;
        case USB_HS_DEVICE_REQUEST_OPERATION_WRITE:
            ...
            break;
        case USB_HS_DEVICE_REQUEST_OPERATION_WRITE_NO_DATA:
...
static void _USB_HS_DEVICE_control_endpoint_in_irq_callback(uint8_t ep_phy) {
    ...
    uint8_t ep0_out_cs_index = USB_HS_DEVICE_RAM_EP_CS_INDEX(0, USB_HS_DEVICE_ENDPOINT_DIRECTION_OUT, USB_HS_DEVICE_ENDPOINT_BUFFER_0);
    uint8_t ep0_in_cs_index = USB_HS_DEVICE_RAM_EP_CS_INDEX(0, USB_HS_DEVICE_ENDPOINT_DIRECTION_IN, USB_HS_DEVICE_ENDPOINT_BUFFER_0);
    uint8_t ep0_out_phy = USB_HS_DEVICE_EP_PHY(0, USB_HS_DEVICE_ENDPOINT_DIRECTION_OUT);
    // Check data phase.
...
static void _USB_HS_DEVICE_data_endpoint_irq_callback(uint8_t ep_phy) {
    ...
    USB_HS_DEVICE_endpoint_buffer_t ep_buffer = _USB_HS_DEVICE_get_endpoint_buffer(ep_phy, USB_HS_DEVICE_ENDPOINT_BUFFER_STATE_FREE);
...
static void _USB_HS_DEVICE_reset_endpoint(uint8_t ep_phy) {
    ...
    usb_hs_device_ctx.physical_endpoints[ep_phy].dma_n_bytes = 0;
...
/*******************************************************************/
static USB_HS_DEVICE_status_t _USB_HS_DEVICE_check_endpoint(USB_HS_DEVICE_endpoint_t* endpoint) {
    // Local variables.
    USB_HS_DEVICE_status_t status = USB_HS_DEVICE_SUCCESS;
    // Check endpoint.
    if (endpoint == NULL) {
    ...
    if ((endpoint->direction) >= USB_HS_DEVICE_ENDPOINT_DIRECTION_LAST) {
...
static USB_HS_DEVICE_endpoint_buffer_t _USB_HS_DEVICE_get_endpoint_buffer(uint8_t ep_phy, USB_HS_DEVICE_endpoint_buffer_state_t buffer_state) {
    ...
    if (USB_HS_DEVICE_EP_NUM(ep_phy) != 0) {
        // Check state.
        if (buffer_state == USB_HS_DEVICE_ENDPOINT_BUFFER_STATE_FREE) {
            ep_buffer = (((USB_HS_DEVICE->EPINUSE) & (0b1 << ep_phy)) == 0) ? USB_HS_DEVICE_ENDPOINT_BUFFER_1 : USB_HS_DEVICE_ENDPOINT_BUFFER_0;
        }
        else {
            ep_buffer = (((USB_HS_DEVICE->EPINUSE) & (0b1 << ep_phy)) == 0) ? USB_HS_DEVICE_ENDPOINT_BUFFER_0 : USB_HS_DEVICE_ENDPOINT_BUFFER_1;
        }
    }
    return ep_buffer;
}
...
static void _USB_HS_DEVICE_write(uint8_t ep_phy, uint8_t ep_num, uint8_t ep_dir) {
    ...
    uint32_t ep_max_packet_size = usb_hs_device_ctx.physical_endpoints[ep_phy].max_packet_size_bytes;
    uint32_t dma_transfer_size = usb_hs_device_ctx.physical_endpoints[ep_phy].dma_transfer_size_bytes;
    uint32_t dma_transfer_count = usb_hs_device_ctx.physical_endpoints[ep_phy].dma_transfer_count_bytes;
    uint32_t dma_remaining_bytes = (dma_transfer_size - dma_transfer_count);
    uint32_t idx = 0;
    // Get RAM address.
    ep_cs_index = USB_HS_DEVICE_RAM_EP_CS_INDEX(ep_num, ep_dir, _USB_HS_DEVICE_get_endpoint_buffer(ep_phy, USB_HS_DEVICE_ENDPOINT_BUFFER_STATE_HW));
    ...
    // Compute single transfer size.
    usb_hs_device_ctx.physical_endpoints[ep_phy].dma_n_bytes = ((dma_remaining_bytes > ep_max_packet_size) ? ep_max_packet_size : dma_remaining_bytes);
    // Copy data in USB RAM.
    for (idx = 0; idx < (usb_hs_device_ctx.physical_endpoints[ep_phy].dma_n_bytes); idx++) {
        usb_hs_device_ram_data.data[ep_ram_data_address + idx] = usb_hs_device_ctx.physical_endpoints[ep_phy].dma_data[dma_transfer_count + idx];
    }
    // Update number of bytes.
    usb_hs_device_ram_data.ep_cs_list[ep_cs_index].n_bytes = usb_hs_device_ctx.physical_endpoints[ep_phy].dma_n_bytes;
    // Activate endpoint.
    usb_hs_device_ram_data.ep_cs_list[ep_cs_index].active = 1;
}
...
USB_HS_DEVICE_status_t USB_HS_DEVICE_register_endpoint(USB_HS_DEVICE_endpoint_t* endpoint) {
    ...
    if ((endpoint->number) == 0) {
        // Check type.
        if ((endpoint->transfer_type) != USB_HS_DEVICE_ENDPOINT_TRANSFER_TYPE_CONTROL) {
        ...
        case USB_HS_DEVICE_ENDPOINT_TRANSFER_TYPE_BULK:
    ...
    if ((endpoint->number != 0) || (endpoint->direction != USB_HS_DEVICE_ENDPOINT_DIRECTION_IN)) {
    ...
    if ((endpoint->number) != 0) {
        USB_HS_DEVICE->EPBUFCFG |= (0b1 << ((USB_HS_DEVICE_ENDPOINT_DIRECTION_LAST * (endpoint->number)) + (endpoint->direction)));
    }
    // Activate out endpoint.
    if ((endpoint->direction) == USB_HS_DEVICE_ENDPOINT_DIRECTION_OUT) {
        // Buffer 0.
    ...
    usb_hs_device_ctx.physical_endpoints[ep_phy].dma_transfer_count_bytes = 0;
    usb_hs_device_ctx.physical_endpoints[ep_phy].dma_transfer_size_bytes = 0;
    usb_hs_device_ctx.physical_endpoints[ep_phy].dma_n_bytes = usb_hs_device_ram_data.ep_cs_list[ep_cs_buffer0_index].n_bytes;
...
USB_HS_DEVICE_status_t USB_HS_DEVICE_write_data(USB_HS_DEVICE_endpoint_t* endpoint, uint8_t* data_in, uint32_t data_in_size) {
    ...
    if (status != USB_HS_DEVICE_SUCCESS) goto errors;
    // Check parameters.
    if ((data_in == NULL) || (data_in_size == 0)) {
        status = USB_HS_DEVICE_ERROR_NULL_PARAMETER;
    }
    // Check if previous transfer has been completed.
    if (usb_hs_device_ctx.physical_endpoints[ep_phy].dma_transfer_count_bytes < usb_hs_device_ctx.physical_endpoints[ep_phy].dma_transfer_size_bytes) {
        status = USB_HS_DEVICE_ERROR_ENDPOINT_BUSY;
        goto errors;
    }
    // Compute endpoint physical index.
    ep_phy = USB_HS_DEVICE_EP_PHY((endpoint->number), (endpoint->direction));
    // Manage DMA transfer.
    usb_hs_device_ctx.physical_endpoints[ep_phy].dma_data = data_in;
    usb_hs_device_ctx.physical_endpoints[ep_phy].dma_transfer_size_bytes = data_in_size;
    usb_hs_device_ctx.physical_endpoints[ep_phy].dma_transfer_count_bytes = 0;
    // Start transfer.
    _USB_HS_DEVICE_write(ep_phy, (endpoint->number), USB_HS_DEVICE_EP_DIR(ep_phy));
errors:
    return status;
}
...
USB_HS_DEVICE_status_t USB_HS_DEVICE_read_data(USB_HS_DEVICE_endpoint_t* endpoint, uint8_t** data_out, uint32_t* data_out_size) {
    ...
    USB_HS_DEVICE_status_t status = USB_HS_DEVICE_SUCCESS;
    uint8_t ep_phy = USB_HS_DEVICE_EP_PHY((endpoint->number), (endpoint->direction));
    uint32_t ep_ram_data_address = USB_HS_DEVICE_RAM_DATA_ADDRESS(usb_hs_device_ctx.physical_endpoints[ep_phy].address_offset[_USB_HS_DEVICE_get_endpoint_buffer(ep_phy, USB_HS_DEVICE_ENDPOINT_BUFFER_STATE_FREE)]);
    // Check state.
    if (usb_hs_device_ctx.flags.init == 0) {
        status = USB_HS_DEVICE_ERROR_UNINITIALIZED;
        goto errors;
