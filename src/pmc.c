...
/*******************************************************************/
static PMC_process_corner_t _PMC_get_process_corner(void) {
    // Local variables.
...
/*******************************************************************/
static void _PMC_set_system_voltage(uint32_t system_voltage_mv) {
    // Local variables.
...
/*******************************************************************/
static PMC_status_t _PMC_set_dc_dc_trim(PMC_dc_dc_profile_t profile) {
    // Local variables.
...
/*******************************************************************/
static PMC_status_t _PMC_set_dc_dc_voltage(PMC_dc_dc_profile_t profile) {
    // Local variables.
    PMC_status_t status = PMC_SUCCESS;
    PMC_process_corner_t part_process_corner = PMC_PROCESS_CORNER_SSS;
    // Read process corner.
...
PMC_status_t PMC_configure_dc_dc(uint32_t system_clock_frequency_hz) {
    ...
    if (system_clock_frequency_hz <= PMC_DCDC_POWER_PROFILE_LOW_MAX_FREQUENCY_HZ) {
        ...
        status = _PMC_set_dc_dc_voltage(PMC_DCDC_POWER_PROFILE_LOW);
    }
        ...
        if (status != PMC_SUCCESS) goto errors;
        status = _PMC_set_dc_dc_voltage(PMC_DCDC_POWER_PROFILE_MEDIUM);
    }
    else {
        status = _PMC_set_dc_dc_trim(PMC_DCDC_POWER_PROFILE_HIGH);
        if (status != PMC_SUCCESS) goto errors;
        status = _PMC_set_dc_dc_voltage(PMC_DCDC_POWER_PROFILE_HIGH);
    }
errors:
    return status;
}
...
PMC_status_t PMC_set_32k_osc_source(PMC_oscillator_t source) {
    ...
    // Check source.
    switch (source) {
    case PMC_OSCILLATOR_FRO32K:
        PMC->RTCOSC32K &= ~(0b1 << 0);
        break;
    case PMC_OSCILLATOR_XTAL32K:
        PMC->RTCOSC32K |= (0b1 << 0);
        break;
    default:
        status = PMC_ERROR_OSCILLATOR;
        goto errors;
    }
errors:
    return status;
...
PMC_status_t PMC_get_oscillator_status(PMC_oscillator_t oscillator, uint8_t* oscillator_is_ready) {
    ...
    PMC_status_t status = PMC_SUCCESS;
    // Check parameter.
    if (oscillator_is_ready == NULL) {
        status = PMC_ERROR_NULL_PARAMETER;
        goto errors;
    }
    // Check source.
    switch (oscillator) {
    case PMC_OSCILLATOR_FRO32K:
        (*oscillator_is_ready) = ((~((PMC->PDRUNCFG0) >> 6)) & 0b1);
        break;
    case PMC_OSCILLATOR_XTAL32K:
        (*oscillator_is_ready) = (((PMC->STATUSCLK) >> 0) & 0b1);
        break;
    default:
        status = PMC_ERROR_OSCILLATOR;
        goto errors;
...
PMC_status_t PMC_get_32k_osc_status(uint8_t* oscillator_is_ready) {
    // Local variables.
    PMC_status_t status = PMC_SUCCESS;
    PMC_oscillator_t clock_source = (((PMC->RTCOSC32K) & (0b1 << 0)) == 0) ? PMC_OSCILLATOR_FRO32K : PMC_OSCILLATOR_XTAL32K;
    // Read status.
    status = PMC_get_oscillator_status(clock_source, oscillator_is_ready);
    if (status != PMC_SUCCESS) goto errors;
errors:
    return status;
}
